    ...
    def timer_callback(self) -> None:
        ...
        try:
            ...
            try:
                ...
                (corners, ids, rejected) = cv2.aruco.detectMarkers(color_image, self.arucoDict, parameters=self.arucoParams)
                if len(corners) > 0:
                    ...
                    for (markerCorner, markerID) in zip(corners, ids):
                        ...
                        cv2.arrowedLine(color_image, (sx, sy), (int(sx + v2[1]), int(sy + v2[0])), (255, 0, 0), 5)
                        cv2.circle(color_image, (center_x, center_y), int(sideLength / 3.0), (0, 255, 0), -1)
                        cv2.putText(color_image, str(int(orientation)) + ", (" + str(center_x) + ", " + str(center_y) + ")", (10, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3, cv2.LINE_AA)

                img_msg = self.br.cv2_to_imgmsg(np.uint8(color_image), encoding='bgr8')
